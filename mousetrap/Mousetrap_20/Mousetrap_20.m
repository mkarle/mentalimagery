%%%AUTHOR: MARK KARLE
clear all
I = imread('ReverseMousetrap_20.png');
[M, N,O] = size(I);
portcullisLocation = [1544 992 39 180];
mouseLocation = [1412 1007 114 168];
%% SEPARATE GROUPS OF OBJECTS
%%%%%%
%find the blue objects (movable)
blueBW = createBlueMask(I);
%close small gaps
blueBW = imdilate(blueBW,strel('disk',8));
...
for i = 1: greenCC.NumObjects
    object(:,:) = 0;
    if(size(greenCC.PixelIdxList{i},1) > 2000)
...
attachmentPointsCC = bwconncomp(greenObjects & blueBW);
...
greenBlackIntersectionPoints = regionprops(greenObjects & blackBW, 'centroid');
greenBlackIntersectionPoints = round(greenBlackIntersectionPoints.Centroid);
clear tmpGreen; 
greenBW = zeros(M,N);
greenBW(greenBlackIntersectionPoints) = 1;
for i = 1 : attachmentPointsCC.NumObjects
...
for i = 1 : blueCC.NumObjects
    if(attachmentMap(i) ~= 0)
        ...
        if tmpAttachmentPoints(attachmentMap(i), :) ~= greenCOM(attachmentMap(i),:)
            oldDistance = sqrt((greenCOM(attachmentMap(i),1) - greenBlackIntersectionPoints(1)) * ...
                (greenCOM(attachmentMap(i),1) - greenBlackIntersectionPoints(1)) + ...
                (greenCOM(attachmentMap(i),2) - greenBlackIntersectionPoints(2)) * ...
                (greenCOM(attachmentMap(i),2) - greenBlackIntersectionPoints(2)));
            newDistance = sqrt((tmpAttachmentPoints(attachmentMap(i),1) - greenBlackIntersectionPoints(1)) * ...
                (tmpAttachmentPoints(attachmentMap(i),1) - greenBlackIntersectionPoints(1)) + ...
                (tmpAttachmentPoints(attachmentMap(i),2) - greenBlackIntersectionPoints(2)) * ...
                (tmpAttachmentPoints(attachmentMap(i),2) - greenBlackIntersectionPoints(2)));
