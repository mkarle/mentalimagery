%%%AUTHOR: MARK KARLE
clear all
I = imread('ReverseMousetrap_01.png');
[M, N,O] = size(I);
portcullisLocation =[1538 1031 33 135];
mouseLocation = [1406 1040 99 135];
...
for i= 1:blueCC.NumObjects
    object(:,:) = 0;
    if(size(blueCC.PixelIdxList{i},1) > 2000)
...
for i = 1 : blueCC.NumObjects
    if(attachmentMap(i) ~= 0)
        ...
        if tmpAttachmentPoints(attachmentMap(i), :) ~= greenCOM(attachmentMap(i),:)
            oldDistance = sqrt((greenCOM(attachmentMap(i),1) - greenCOM(attachmentMap(3),1)) * ...
                (greenCOM(attachmentMap(i),1) - greenCOM(attachmentMap(3),1)) + ...
                (greenCOM(attachmentMap(i),2) - greenCOM(attachmentMap(3),2)) * ...
                (greenCOM(attachmentMap(i),2) - greenCOM(attachmentMap(3),2)));
            newDistance = sqrt((tmpAttachmentPoints(attachmentMap(i),1) - greenCOM(attachmentMap(3),1)) * ...
                (tmpAttachmentPoints(attachmentMap(i),1) - greenCOM(attachmentMap(3),1)) + ...
                (tmpAttachmentPoints(attachmentMap(i),2) - greenCOM(attachmentMap(3),2)) * ...
                (tmpAttachmentPoints(attachmentMap(i),2) - greenCOM(attachmentMap(3),2)));
