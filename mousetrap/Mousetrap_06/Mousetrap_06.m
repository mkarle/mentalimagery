%%%AUTHOR: MARK KARLE
clear all
I = imread('ReverseMousetrap_06.png');
[M, N,O] = size(I);
...
for i= 1:blueCC.NumObjects
    object(:,:) = 0;
    if(size(blueCC.PixelIdxList{i},1) > 1000)
        object(blueCC.PixelIdxList{i}) = 1;
        blueObjects(:,:,i) = object;
    end
end
blueObjects = logical(blueObjects);
%get rid of empty blues
blueCount = 0;
tmpblue = 0;
blueBW = zeros(M,N);
for i = 1:blueCC.NumObjects
    if any(any(blueObjects(:,:,i)))
        blueCount = blueCount + 1;
        tmpblue(1:M,1:N, blueCount) = blueObjects(:,:,i);
        blueBW = blueBW | tmpblue(1:M,1:N,blueCount);
    end
end
blueObjects = tmpblue;
blueCC.NumObjects = blueCount;
clear tmpblue;
...
for i = 1: greenCC.NumObjects
    object(:,:) = 0;
    if(size(greenCC.PixelIdxList{i},1) > 200)
...
blueCOM = regionprops(blueBW, 'centroid');
...
for i = 1:blueCC.NumObjects
    if ~blueIsFixed(i) && ~objectWasPulled(i)
        if ~((blackBW & blueObjects(:,:,i)) | (blueObjects(:,:,i) & blueObjects(:,:,1)))
